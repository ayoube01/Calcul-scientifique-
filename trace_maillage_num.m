function trace_maillage_num(nbn,nbe,nba,coord,tri,ar,refn,reft,refa)
...
title(sprintf('NUMEROS du Maillage Eléments Finis de type P1 (%d triangles, %d noeuds, %d aretes)',nbe,nbn,nba))
xlabel('axe Ox'); ylabel('axe Oy'); axis equal;

for i=1:length(xt) % noeuds
  h=text(xt(i),yt(i),num2str(i)); 
  set(h,'Color','r');
end
